...
public class BitmapPool {
	...
	public static void loadAll(Context context) {
		...
		pool.put(R.drawable.backboard, Bitmap.createScaledBitmap(bb,
				(int) Game.Constant.BACKBOARD_WIDHT,
				(int) Game.Constant.BACKBOARD_HEIGHT, true));
		bb.recycle();
		bb = null;
		// 背景
		...
		pool.put(R.drawable.background, Bitmap.createScaledBitmap(bg,
				(int) Game.Constant.SCREEN_WIDTH,
				(int) Game.Constant.SCREEN_HEIGHT, true));
		bg.recycle();
		bg = null;
		// 篮筐
		for (int i = 0; i < 5; ++i) {
			Bitmap hoop = BitmapFactory.decodeResource(res, R.drawable.hoop00
					+ i);
			pool.put(R.drawable.hoop00 + i, Bitmap.createScaledBitmap(hoop,
					(int) Game.Constant.HOOP_WIDTH,
					(int) Game.Constant.HOOP_HEIGHT, true));
			hoop.recycle();
			hoop = null;
		}
		...
		// 篮球
		for (int i = 0; i < 5; ++i) {
			Bitmap ball = BitmapFactory.decodeResource(res, R.drawable.ball000
					+ i);
			pool.put(R.drawable.ball000 + i, Bitmap.createScaledBitmap(ball,
					(int) Game.Constant.BALL_RADIUS * 2,
					(int) Game.Constant.BALL_RADIUS * 2, true));
			ball.recycle();
			ball = null;
		}
		...
		pool.put(R.drawable.leaf, Bitmap.createScaledBitmap(leaf,
				(int) Game.Constant.LEAF_WIDTH,
		...
		// 时间数字
		for (int i = 0; i < 10; ++i) {
			Bitmap timeNum = BitmapFactory.decodeResource(res, R.drawable.time0
					+ i);
			pool.put(R.drawable.time0 + i, Bitmap.createScaledBitmap(timeNum,
					(int) Game.Constant.TIME_NUM_WIDTH,
					(int) Game.Constant.TIME_NUM_HEIGHT, true));
			timeNum.recycle();
			timeNum = null;
		}
		...
		// 分数数字
		for (int i = 0; i < 10; ++i) {
			Bitmap scoreNum = BitmapFactory.decodeResource(res,
					R.drawable.score0 + i);
			pool.put(R.drawable.score0 + i, Bitmap.createScaledBitmap(scoreNum,
					(int) Game.Constant.SCORE_NUM_WIDTH,
		...
		// 进球提示
		for (int i = 0; i < 2; ++i) {
			Bitmap hint = BitmapFactory.decodeResource(res, R.drawable.hint0
					+ i);
			pool.put(R.drawable.hint0 + i, Bitmap.createScaledBitmap(hint,
					(int) Game.Constant.HINT_WIDTH,
					(int) Game.Constant.HINT_HEIGHT, true));
			hint.recycle();
			hint = null;
		}
		...
		// 关卡的数字
		for (int i = 0; i < 4; i++) {
			Bitmap level = BitmapFactory.decodeResource(res, R.drawable.level1
					+ i);
			pool.put(R.drawable.level1 + i, Bitmap.createScaledBitmap(level,
					(int) Game.Constant.LEVEL_WIDTH,
					(int) Game.Constant.LEVEL_HEIGHT, true));
			level.recycle();
			level = null;
		}
		...
		pool.put(R.drawable.level, Bitmap.createScaledBitmap(level,
				(int) Game.Constant.LEVEL_WIDTH * 5,
				(int) Game.Constant.LEVEL_HEIGHT, true));
		level.recycle();
		level = null;
	}
